...
class OrderMessageButtons(disnake.ui.View):
    ...
    async def leave_review_button(self, button: disnake.ui.Button, ctx):  # ctx: disnake.AppCmdInter
        thread = await ctx.channel.create_thread(name=f"{ctx.author.name}'s отзыв", type=disnake.ChannelType.private_thread)

        embed = utils.create_embed(title="Продолжение создания", color=SSBot.DEFAULT_COLOR, content=f"Для того чтобы продолжить создание отзыва перейдите в ветку <#{thread.id}>")
    ...
    async def work_examples_button(self, button: disnake.ui.Button, ctx):  # ctx: disnake.AppCmdInter
